...
- UI 中的绘画速度 / 延迟 / 画布缩放控制与核心参数完全对应，并支持热键校准画布
...
- **绘画速度**：界面 1~100 数值线性映射到落点延时，速度 1≈0.008s、速度 100≈0.0008s（数值越大越快）。
- **起笔延迟**：开始绘制前的等待时间（秒），用于切换到目标窗口。
- **画布缩放**：在用户框选的画布区域内，按比例缩放落笔区域（10%~200%）。
- **热键**：F8 记录画布左上角、F9 记录右下角，ESC 可随时中断。
